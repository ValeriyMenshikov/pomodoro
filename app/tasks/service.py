...
class TaskService:
    ...
    async def create_task(
        self, task: CreateOrUpdateTaskSchema, user_id: int
    ) -> TaskSchema:
        task = await self.task_repository.create_task(task=task, user_id=user_id)
        return TaskSchema.model_validate(task)

    async def update_task(
        self, task: CreateOrUpdateTaskSchema, task_id: int, user_id: int
    ) -> TaskSchema:
        user_task = await self.task_repository.get_user_task(
